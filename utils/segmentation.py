...
from net import *
from net.losses import StdLoss, YIQGNGCLoss, GradientLoss, ExtendedL1Loss, GrayLoss, ExclusionLoss
...
""" namedtuple('tuple name', ('field1', 'field2'), ...)
    used for saving group of image output
"""
SegmentationResult = namedtuple(
    "SegmentationResult", ['mask', 'learned_mask', 'left', 'right', 'psnr'])


class Segmentation(object):
    def __init__(self, image_name, image,
                 plot_during_training=True, plot_after_step=True,
                 ...
                 output_path='output/'):
        """
        plot_during_training: showing training metrics during training iterations
        input_depth: input noise layers
        output_depth: input image channels
        """
        self.image = image
        self.input_depth = input_depth
        self.output_depth = output_depth
        self.psnr_goal = psnr_goal
        if bg_hint is None or fg_hint is None:
            raise Exception("Hints must be provided")
        self.image_name = image_name
        self.plot_during_training = plot_during_training
        self.plot_after_step = plot_after_step
    ...
    def _init_images(self):
        """actually no resize, just [image]
        """
        self.images = get_imresize_downsampled(self.image, downsampling_factor=self.downsampling_factor,
                        downsampling_number=0)  # just [image]
        self.images_torch = [np_to_torch(image).type(
            torch.cuda.FloatTensor) for image in self.images]

        if self.bg_hint is not None:
            assert self.bg_hint.shape[1:] == self.image.shape[1:]
            self.bg_hints = get_imresize_downsampled(self.bg_hint,
                    downsampling_factor=self.downsampling_factor,
                    downsampling_number=self.downsampling_number)
            self.bg_hints_torch = [np_to_torch(bg_hint).type(
                torch.cuda.FloatTensor) for bg_hint in self.bg_hints]
        else:
            self.bg_hints = None

        if self.fg_hint is not None:
            assert self.fg_hint.shape[1:] == self.image.shape[1:]
            self.fg_hints = get_imresize_downsampled(self.fg_hint,
                                                     downsampling_factor=self.downsampling_factor,
                                                     downsampling_number=self.downsampling_number)
            self.fg_hints_torch = [np_to_torch(fg_hint).type(
                torch.cuda.FloatTensor) for fg_hint in self.fg_hints]
        else:
            self.fg_hints = None
    ...
    def _initialize_any_step(self, iteration):
        ...
        connect with net to give output net_output
        '''
        ...
        # creates left_net_inputs and right_net_inputs by adding small norm noise
        ...
        # applies the nets
        self.left_net_outputs = [self.left_net(
            left_net_input) for left_net_input in left_net_inputs]
        self.right_net_outputs = [self.right_net(
            right_net_input) for right_net_input in right_net_inputs]
        self.mask_net_outputs = [self.mask_net(
            mask_net_input) for mask_net_input in mask_net_inputs]
    ...
    def finalize(self):
        ...
        # save_image(self.image_name + "_original",
        #            self.images[0], self.output_path)
        # save_image(self.image_name + "_fg_bg",
        #            ((self.fg_hint - self.bg_hint) + 1) / 2, self.output_path)
        save_image(self.image_name + "_learned_mask",
                   self.best_result.learned_mask, self.output_path, suffix='visual')
        save_image(self.image_name + "_mask",
                   self.best_result.mask, self.output_path, suffix='final')

    def _update_result_closure(self):
        """
        saving the `self.best_result` using SegmentationResult namedtuple
        """
    ...
    def finalize_first_step(self):
        ...
        save_image(self.image_name + "_hint1_masked",
                   self.bg_hint * self.image)
        save_image(self.image_name + "_hint2_masked",
                   self.fg_hint * self.image)
    ...
    def _step2_optimize_without_hints(self, iteration):
        for left_out, right_out, mask_out, original_image_torch in zip(self.left_net_outputs,
            ...
            self.total_loss += (0.01 * (iteration // 100)
                                ) * self.current_gradient
        self.total_loss.backward(retain_graph=True)

    def _step2_optimize_with_hints(self, iteration):
        iters = 1000
        # guidance is relaxed after `iters` iterations
        if iteration <= iters:
            self.total_loss += sum(self.extended_l1_loss(left_net_output, image_torch, fg_hint) for
                                   left_net_output, fg_hint, image_torch
                                   in zip(self.left_net_outputs, self.fg_hints_torch, self.images_torch))
        ...
        for left_out, right_out, mask_out, original_image_torch in zip(self.left_net_outputs,
                                                                       ...
                                                                       self.mask_net_outputs,
                                                                       self.images_torch):
            self.total_loss += self.l1_loss(mask_out * left_out + (1 - mask_out) * right_out,
                                                  original_image_torch)
            self.current_gradient = self.gray_loss(mask_out)
            # self.current_gradient = self.gradient_loss(mask_out)
            # self.exclusion = self.exclusion_loss(left_out, right_out)
            iteration = min(iteration, iters)
            self.total_loss += (0.0001 * (iteration // 100)) * self.current_gradient
        self.total_loss.backward(retain_graph=True)

    def _finalize_iteration(self, step=1):
        """
        calculate psnr between original and network outputs
        if `psnr` greater than 30
        set `second_step_done` to True
        """
        ...
        self.current_psnr = compare_psnr(original_image, 
                mask_out_np*left_out_np + (1-mask_out_np)*right_out_np)
        # run only in the second step
        if self.current_psnr > self.psnr_goal and step == 2:
    ...
    def _iteration_plot_closure(self, iter_number, step):
        ...
        if iter_number % self.show_every == self.show_every - 1:
            ...
            self._save_in_steps(iter_number, step)
        ...
        for i, (left_out, right_out, mask_out, image) in enumerate(zip(self.left_net_outputs,
            ...
            plot_image_grid("learned_mask_{}_{}".format(name, i),
                            [np.clip(mask_out_np, 0, 1),
                             1 - np.clip(mask_out_np, 0, 1)],
                            output_path=self.output_path)

            plot_image_grid("learned_image_{}_{}".format(name, i),
                            [image,
                            np.clip(mask_out_np*torch_to_np(left_out) + (1-mask_out_np)*torch_to_np(right_out),0, 1), 
                            ],
                            output_path=self.output_path)

    def _save_in_steps(self, name, step):
        self._fix_mask_2(0.66)
        mask=self.fixed_masks[0]
        save_image("fixed_mask_{}_{:04d}".format(step, name),
                   mask, self.output_path, suffix='fixed_mask')

        mask_out = self.mask_net_outputs[0]
        save_image("mask_{}_{:04d}".format(step, name),
                   torch_to_np(mask_out), self.output_path, suffix='mask')

        left_out = self.left_net_outputs[0]
        right_out = self.right_net_outputs[0]
        plot_image_grid("left_right_{}_{:04d}".format(step, name),
                [np.clip(torch_to_np(left_out), 0, 1),
                 np.clip(torch_to_np(right_out), 0, 1)],
                output_path=self.output_path + '/left_right/')

        recon = mask_out * left_out + (1 - mask_out) * right_out
        save_image("recon_{}_{:04d}".format(step, name),
                   torch_to_np(recon), self.output_path, suffix='reconstruct')