...
def save_image(name, image_np, output_path="output/", suffix='final'):
    """save in output/<suffix>/0000.png
    """
    p = np_to_pil(image_np)
    name_list = name.split('/')
    os.makedirs(os.path.join(output_path, suffix), exist_ok=True)
    save_path = os.path.join(output_path, suffix, "{}.jpg".format(name_list[-1]))
    p.save(save_path)
